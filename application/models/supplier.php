...
{	
	...
	*/
	function get_all($limit=10000, $offset=0,$col='company_name',$order='asc')
	{
		...
		$data=$this->db->query("SELECT * , 
							(SELECT sa.balance 
							FROM ". $this->db->dbprefix('store_accounts') ." AS sa 
							WHERE sa.deleted=0 AND sa.supplier_id=". $suppliers.".person_id
	...
	*/
	...
	{
		$this->db->select("*, (SELECT sa.balance FROM ". $this->db->dbprefix('store_accounts') ." AS sa WHERE sa.deleted=0 AND sa.supplier_id=".$supplier_id." ORDER BY `date` DESC LIMIT 1) AS balance");
		$this->db->from('suppliers');	
		$this->db->join('people', 'people.person_id = suppliers.person_id');
	...
	*/
	function search($search, $limit=20,$offset=0,$column='last_name',$orderby='asc')
	{
		if ($this->config->item('speed_up_search_queries'))
		{		
			$balance_field = "(SELECT sa.balance FROM ". $this->db->dbprefix('store_accounts') ." AS sa WHERE sa.deleted=0 AND sa.supplier_id=".$this->db->dbprefix('people').".person_id ORDER BY `date` DESC LIMIT 1) AS balance";	
			$query = "
			select *
			from (
	           	(select ".$this->db->dbprefix('people').".*, ".$this->db->dbprefix('suppliers').".company_name, ".$this->db->dbprefix('suppliers').".deleted, ".$this->db->dbprefix('suppliers').".account_number, ".$balance_field."
		...
		{
			$this->db->select("*, (SELECT sa.balance FROM ". $this->db->dbprefix('store_accounts') ." AS sa WHERE sa.deleted=0 AND sa.supplier_id=".$this->db->dbprefix('people').".person_id ORDER BY `date` DESC LIMIT 1) AS balance");
			$this->db->from('suppliers');
	 		$this->db->join('people','suppliers.person_id=people.person_id');
			$this->db->where("(first_name LIKE '%".$this->db->escape_like_str($search)."%' or 
			last_name LIKE '%".$this->db->escape_like_str($search)."%' or 
	...
	function get_current_balence($supplier_id){
		$this->db->select("store_accounts.balance");
