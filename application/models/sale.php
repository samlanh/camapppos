...
class Sale extends CI_Model
{
	...
	function save ($items,$customer_id,$employee_id,$comment,$payments,$sale_id=false, $suspended = 0)
	{
		...
		{
			$payment_types= $payment_types. $payment['payment_date'] ." ". $payment['payment_type'].': '.to_currency($payment['payment_amount']).'<br />';
		}
	...
	function get_all_payments(){
		$sql = "SELECT 
					...
					p.payment_date
				FROM
				   ".$this->db->dbprefix('sales_payments')." AS p
					INNER JOIN 	".$this->db->dbprefix('sales')." AS s ON (s.sale_id=p.sale_id) 
				WHERE s.deleted = 0 AND s.suspended = 1
				ORDER BY p.sale_id, p.payment_date DESC";
		//echo $sql; exit;
	...
	function get_all_suspended()
	{
		...
		return $this->db->get(); */
		//Change By Sovann to get more field total and pay
		$sql = "SELECT 
				  s.sale_time,
				  s.customer_id,
				  s.employee_id,
				  ...
				  s.deleted,
				  s.suspended,
				  (SELECT SUM((si.`quantity_purchased` * si.`item_unit_price`) - (si.`quantity_purchased` * si.`item_unit_price` * (si.`discount_percent` / 100))) FROM ".$this->db->dbprefix('sales_items')." AS si WHERE s.`sale_id` = si.`sale_id`) AS total,
				  (SELECT IF(ISNULL(SUM(sp.`payment_amount`)),0, SUM(sp.`payment_amount`)) FROM ".$this->db->dbprefix('sales_payments')." AS sp WHERE s.`sale_id` = sp.`sale_id`) AS pay
				FROM
	...
	function get_payments_sale_id($exp)
	{
		$sql = "SELECT
				  	sp.sale_id,
					sp.payment_type,
					...
					sp.payment_date
				FROM
				  ".$this->db->dbprefix('sales_payments')." AS sp
				  INNER JOIN 	".$this->db->dbprefix('sales')." AS s ON (s.sale_id=sp.sale_id)";
	
		$where = " WHERE s.deleted = 0
				  AND s.suspended = 1";
		
		$orderby = " ORDER BY sp.sale_id, sp.payment_date DESC";
		
		if(is_numeric($exp)){
			$where .= " AND sp.sale_id = " . $exp;
		}
		else{
			$sql .= " INNER JOIN ".$this->db->dbprefix('people')." AS p	ON (s.customer_id = p.person_id)";
			$where .= " AND (
							p.first_name LIKE '%".$this->db->escape_like_str($exp)."%' or
							p.last_name LIKE '%".$this->db->escape_like_str($exp)."%' or
							CONCAT(p.`first_name`,' ',p.`last_name`) LIKE '%".$this->db->escape_like_str($exp)."%'
							)";
		}
		$sql .= $where . $orderby;
		//echo $sql; exit;
	...
	function get_suspended_sale_id($exp)
	{
		$sql = "SELECT 
				  s.sale_time,
				  s.customer_id,
		...
		$where = " WHERE s.deleted = 0 
				  AND s.suspended = 1";		
		$groupby = " GROUP BY s.`sale_id`";
		$orderby = " ORDER BY s.sale_id";
		$having = "";
		if(is_numeric($exp)){					
			$having = " HAVING sale_id = " . $exp;		
		}
		else{
			$sql .= " INNER JOIN ".$this->db->dbprefix('people')." AS p
							ON (s.customer_id = p.person_id)";
			$where .= " AND (
							p.first_name LIKE '%".$this->db->escape_like_str($exp)."%' or
							p.last_name LIKE '%".$this->db->escape_like_str($exp)."%' or
							CONCAT(p.`first_name`,' ',p.`last_name`) LIKE '%".$this->db->escape_like_str($exp)."%'
		...
		$sql .= $where . $groupby . $having . $orderby;
