...
class Sales extends Secure_area
{
	...
	{		
		...
		if( !$this->sale_lib->add_payment( $payment_type, $payment_amount, date('Y-m-d H:i:s') ) )
		{
	...
	function complete()
	{
		...
		$data['sale_id']='POS '.$this->Sale->save($data['cart'], $customer_id,$employee_id,$comment,$data['payments'], $this->sale_lib->get_suspended_sale_id(), 0);
	...
	function suspend()
	{
		...
		//SAVE sale to database
		$data['sale_id']='POS '.$this->Sale->save($data['cart'], $customer_id,$employee_id,$comment,$data['payments'], $sale_id, 1);
		if ($data['sale_id'] == 'POS -1')
		{
			$data['error_message'] = lang('sales_transaction_failed');
		}
