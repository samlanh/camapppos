...
{	
	...
	function specific_supplier_store_accounts_input()
	{
		$data = $this->_get_common_report_data();
		$data['specific_input_name'] = lang('reports_supplier');
	...
	function specific_supplier_store_accounts($start_date, $end_date, $supplier_id, $sale_type, $export_excel=0)
	{
		...
		$model = $this->Specific_supplier_store_accounts;		
		$model->setParams(array('start_date'=>$start_date, 'end_date'=>$end_date, 'supplier_id' =>$supplier_id, 'sale_type' => $sale_type));
		...
		{
			$debit = ($row['transaction_amount']>0)?to_currency($row['transaction_amount']):null;
			$credit = ($row['transaction_amount']<0)?to_currency($row['transaction_amount']):null;
			$receive = (!is_null($row['receiving_id']))?'RECV '.$row['receiving_id']:null;
			$tmp = strtotime($row['date']);
			$tabular_data[] = array(
										array('data'=>($i+1), 'align' => 'center'),
										array('data'=>date(get_date_format(), $tmp)." ".date(get_time_format(), $tmp), 'align' => 'center'), 
										array('data'=>$receive, 'align' => 'left'), 
